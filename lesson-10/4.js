...
const validateFunc = (arr) => {
    const result = arr.some(element => typeof element !== 'function' || element() === undefined);
    return result;
}

const compose = (...params) => {
    if (validateFunc(params)) {
        throw new Error(`argument should be a function and it should return result`)
    };

    const result = (initialValue = '') => {
        return params.reduceRight((previousValue, currentValue) => currentValue(previousValue), initialValue);
    };
    return result;
};
